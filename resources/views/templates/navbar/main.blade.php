<nav class="w-full px-4 py-4 flex justify-between items-center bg-white">
    <a class="text-3xl font-bold leading-none" href="#">
        <img class="w-10" src="{{asset('img/dragon.svg')}}" alt="Dragon" />
    </a>
    <div class="lg:hidden">
        <button class="navbar-burger flex items-center text-drdarkbrown p-3">
    ...
    <ul class="hidden relative lg:flex lg:mx-auto lg:flex lg:items-center lg:w-auto lg:space-x-6">
        @foreach($pages as $index => $page)
            @if($index != 0)
                @include('templates.navbar.linkseparator')
            @endif
            @include('templates.navbar.largelink', $page)
        @endforeach
    </ul>
    ...
    <nav class="fixed top-0 left-0 bottom-0 flex flex-col w-5/6 max-w-sm py-6 px-6 bg-white border-r overflow-y-auto">
        <div class="flex items-center mb-8">
            ...
            <button class="navbar-close">
                <svg class="h-6 w-6 text-drdarkbrown cursor-pointer hover:text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div>
            <ul>
                @foreach($pages as $page)
                    @include('templates.navbar.smalllink', $page)
                @endforeach
            </ul>
        </div>
        <div class="mt-auto">
